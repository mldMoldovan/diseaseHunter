...
public class BossFiveSucker extends GameObject{
    
    private Handler handler;
    private Random r = new Random();
    private int TimeSucks=80;
    private int TimeSucks2=50;
    private float Sucker5Health = 100;
    private float blingreenvalue = 254f;
    private Color col=Color.black;
    
    public BossFiveSucker(int x, int y, ID id, Handler handler){
        super(x,y,id);
    ...
    public void tick(){
        x+=velX;
        y+=velY;
        Sucker5Health = ChewingGumSimulator.clamp(Sucker5Health,0,100);
        blingreenvalue = Sucker5Health;
        ...
        if(TimeSucks2 <= 0) {
            ...
            collision();
            if(Sucker5Health==0)
        {   ChewingGumSimulator.trophiesStatus[9]=true;
        ...
        if(tempObject.getId() == ID.PlayerRocket || tempObject.getId() == ID.PlayerRocketH)
        {
            if(getBounds().intersects(tempObject.getBounds()))
            {
                ...
                ChewingGumSimulator.playerXP+=80;}
                Sucker5Health--;
            }
        }
        ...
        if(tempObject.getId() == ID.ParticleDisengage2)
        {
            ...
            {
                if(ChewingGumSimulator.playerXP<100000){
                ChewingGumSimulator.playerXP+=360;}
                handler.removeObject(this);
        }
    ...
    public void render(Graphics g){
        ...
        g.fillRect((int)x+5, (int)y-20,(int) Sucker5Health *2/10, 3);
        g.setColor(Color.white);
        g.drawRect((int)x+5, (int)y-20,(int) Sucker5Health *2/10, 3);
