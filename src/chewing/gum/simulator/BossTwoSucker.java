...
public class BossTwoSucker extends GameObject{
    
    private Handler handler;
    private Random r = new Random();
    private int TimeSucks=80;
    private int TimeSucks2=50;
    private int Sucker2Health = 100;
    private float blingreenvalue = 254f;
    private Color col=Color.black;
    
    public BossTwoSucker(int x, int y, ID id, Handler handler){
        super(x,y,id);
    ...
    public void tick(){
        ...
        if(TimeSucks2 <= 0) {
            ...
            collision();
            if(Sucker2Health==0)
        {   ChewingGumSimulator.trophiesStatus[6]=true;
            handler.addObject(new SilverSoftTreasure((int)x+10,(int) y+50, ID.SilverSoftTreasure,handler));
        ...
        if(tempObject.getId() == ID.PlayerRocket || tempObject.getId() == ID.PlayerRocketH)
        {
            if(getBounds().intersects(tempObject.getBounds()))
            {
                ...
                ChewingGumSimulator.playerXP+=9;}
                Sucker2Health--;
            }
        }
        ...
        if(tempObject.getId() == ID.PlayerLightning)
        {
            if(getBounds().intersects(tempObject.getBounds()))
            {
                ...
                if(ChewingGumSimulator.playerXP<100000){
                ChewingGumSimulator.playerXP+=27;}
                Sucker2Health-=2;
            }
        }
    ...
    public void render(Graphics g){
        ...
        g.fillRect((int)x+5, (int)y-20,(int) Sucker2Health *2/10, 3);
        g.setColor(Color.white);
        g.drawRect((int)x+5, (int)y-20,(int) Sucker2Health *2/10, 3);
