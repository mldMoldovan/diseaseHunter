...
public class BossThreeSucker extends GameObject{
    
    private Handler handler;
    private Random r = new Random();
    private int TimeSucks=80;
    private int TimeSucks2=50;
    private float Sucker3Health = 100;
    private float blingreenvalue = 254f;
    private Color col=Color.black;
    
    public BossThreeSucker(int x, int y, ID id, Handler handler){
        super(x,y,id);
    ...
    public void tick(){
        x+=velX;
        y+=velY;
        Sucker3Health = ChewingGumSimulator.clamp(Sucker3Health,0,100);
        blingreenvalue = Sucker3Health;
        ...
        if(TimeSucks2 <= 0) {
            ...
            collision();
            if(Sucker3Health==0)
        {   ChewingGumSimulator.trophiesStatus[7]=true;
        ...
        if(tempObject.getId() == ID.PlayerRocket || tempObject.getId() == ID.PlayerRocketH)
        {
            if(getBounds().intersects(tempObject.getBounds()))
            {
                ...
                ChewingGumSimulator.playerXP+=18;}
                Sucker3Health--;
            }
        }
        ...
        if(tempObject.getId() == ID.PlayerLightning)
        {
            if(getBounds().intersects(tempObject.getBounds()))
            {
                ...
                if(ChewingGumSimulator.playerXP<100000){
                ChewingGumSimulator.playerXP+=53;}
                Sucker3Health-=2;
            }
        }
    ...
    public void render(Graphics g){
        ...
        g.fillRect((int)x+5, (int)y-20,(int) Sucker3Health *2/10, 3);
        g.setColor(Color.white);
        g.drawRect((int)x+5, (int)y-20,(int) Sucker3Health *2/10, 3);
