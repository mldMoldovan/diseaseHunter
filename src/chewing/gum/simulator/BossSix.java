...
public class BossSix extends GameObject{
    ...
    private float Boss6Health = 200;
    private float anothergreenvalue = 254f;
    
    
    
    public BossSix(float x, float y, ID id, Handler handler){
        super(x,y,id);
    ...
    public void tick(){
        x+=velX;
        y+=velY;
        Boss6Health = ChewingGumSimulator.clamp(Boss6Health,0,200);
        anothergreenvalue = Boss6Health;
        ...
        collision();
        if(Boss6Health==70 && ChewingGumSimulator.gameState!=ChewingGumSimulator.STATE.AdventureSix)
        {   if(ChewingGumSimulator.gameState!=ChewingGumSimulator.STATE.AdventureSix){
            handler.addObject(new BossSixSucker((ChewingGumSimulator.WIDTH / 2)-48,0,ID.BossSixSucker,handler));
        }
        }
        
        if(Boss6Health==0)
        {   ChewingGumSimulator.trophiesStatus[10]=true;
            
            if(ChewingGumSimulator.gameState==ChewingGumSimulator.STATE.AdventureSix){
         
               ChewingGumSimulator.gameState=ChewingGumSimulator.STATE.AdventureSixWon;
               ChewingGumSimulator.adventureStatus[2]=true;
               ChewingGumSimulator.adventureStatus[6]=true;
    ...
    public void render(Graphics g){
        ...
        g.fillRect((int)x+5, (int)y-20,(int) Boss6Health /10, 3);
        g.setColor(Color.white);
        g.drawRect((int)x+5, (int)y-20,(int) Boss6Health /10, 3);
    }
