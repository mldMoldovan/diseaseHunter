...
public class BossEight extends GameObject{
    ...
    private float Boss8Health = 200;
    private float anothergreenvalue = 254f;
    
    
    
    public BossEight(float x, float y, ID id, Handler handler){
        super(x,y,id);
    ...
    public void tick(){
        x+=velX;
        y+=velY;
        Boss8Health = ChewingGumSimulator.clamp(Boss8Health,0,200);
        anothergreenvalue = Boss8Health;
        ...
        collision();
        if(Boss8Health==70 && ChewingGumSimulator.gameState!=ChewingGumSimulator.STATE.AdventureEight)
        {   if(ChewingGumSimulator.gameState!=ChewingGumSimulator.STATE.AdventureEight){
            handler.addObject(new BossEightSucker((ChewingGumSimulator.WIDTH / 2)-48,0,ID.BossEightSucker,handler));
        }
        }
        
        if(Boss8Health==0)
        {   ChewingGumSimulator.trophiesStatus[12]=true;
            
            if(ChewingGumSimulator.gameState==ChewingGumSimulator.STATE.AdventureEight){
         
               ChewingGumSimulator.gameState=ChewingGumSimulator.STATE.AdventureEightWon;
               ChewingGumSimulator.adventureStatus[2]=true;
               ChewingGumSimulator.adventureStatus[8]=true;
    ...
    public void render(Graphics g){
        ...
        g.fillRect((int)x+5, (int)y-20,(int) Boss8Health /10, 3);
        g.setColor(Color.white);
        g.drawRect((int)x+5, (int)y-20,(int) Boss8Health /10, 3);
    }
