...
public class BossSucker extends GameObject{
    
    private Handler handler;
    private Random r = new Random();
    private int TimeSucks=80;
    private int TimeSucks2=50;
    private float SuckerHealth = 100;
    private float blingreenvalue = 254f;
    private Color col=Color.black;
    
    public BossSucker(int x, int y, ID id, Handler handler){
        super(x,y,id);
    ...
    public void tick(){
        x+=velX;
        y+=velY;
        SuckerHealth = ChewingGumSimulator.clamp(SuckerHealth,0,100);
        blingreenvalue = SuckerHealth;
        ...
        if(TimeSucks2 <= 0) {
            ...
            if(SuckerHealth==0)
        {   
            if(ChewingGumSimulator.gameState==ChewingGumSimulator.STATE.Tutorial && ChewingGumSimulator.tutorialStages[10]==true && ChewingGumSimulator.tutorialStages[11]==false){
                ChewingGumSimulator.tutorialStages[11]=true;
            }
            ChewingGumSimulator.trophiesStatus[5]=true;
    ...
    public void render(Graphics g){
        ...
        g.fillRect((int)x+5, (int)y-20,(int) SuckerHealth *2/10, 3);
        g.setColor(Color.white);
        g.drawRect((int)x+5, (int)y-20,(int) SuckerHealth *2/10, 3);
